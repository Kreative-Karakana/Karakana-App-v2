...
void main() {
  group('BusinessManagementProvider.updateTransaction', () {
    ...
    test('failed edit surfaces an error without corrupting loaded state',
        () async {
      final service = _FakeService(
        transactionsPage: _page([_transaction(1, amount: '1000.00')]),
        dashboard: _dashboard(),
      )..failNextUpdate = DioException(
          requestOptions: RequestOptions(path: '/api/v1/businesses/transactions/1/'),
          response: Response(
            requestOptions: RequestOptions(path: ''),
            statusCode: 400,
            data: {'amount': ['Kiasi lazima kiwe zaidi ya sifuri.']},
          ),
        );
      final provider = BusinessManagementProvider(service: service);
      await provider.loadTransactions();
