...
const subscriptionProduct = 'com.kreativekarakana.karakana.subscription.monthly';
...
void main() {
  group('RestorePurchasesProvider', () {
    test(
      ...
      expect(store.registeredKinds[subscriptionProduct],
      ...
      for (final id in [courseProduct, ebookProduct, subscriptionProduct]) {
        final loadIndex = store.calls
            .indexWhere((c) => c.startsWith('loadProducts:') && c.contains(id));
        expect(loadIndex, allOf(greaterThanOrEqualTo(0), lessThan(restoreIndex)),
    ...
    test('paginates through every page of enrolled courses', () async {
      final store = _RecordingStore();
    ...
    test('reports nothingToRestore when the store has nothing to restore',
        () async {
      final store = _RecordingStore()
    ...
    test('a failing course fetch does not block eBook/subscription restore',
        () async {
      final store = _RecordingStore();
      final provider = RestorePurchasesProvider(
        store: store,
...
EbookPurchase _purchase(String? appleProductId,
        {int id = 1, bool successful = true}) =>
    EbookPurchase(
      id: id,
